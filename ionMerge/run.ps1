...
$param = @{
    ...
    Body    = "SELECT $object FROM $fromDB AS e INNER JOIN `"HCM_EmployeeAddress`" AS ea ON `"e`".`"Employee`" = `"ea`".`"Employee`" WHERE e.`"RelationshipToOrganization`" = 'EMPLOYEE' AND e.`"RelationshipStatus`" = 'ACTIVE'"
}
